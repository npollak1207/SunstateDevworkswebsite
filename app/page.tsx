...
function ProjectCard({ p, isExternal }: { p: { title: string; cat: string; tag: string; accent: string; num: string; stat: string; desc: string }, isExternal: boolean }) {
  return (
      <div className="card-lift" style={{ background: 'var(--navy)', padding: '36px 32px', borderBottom: `3px solid ${p.accent}`, position: 'relative', overflow: 'hidden', cursor: 'pointer', height: '100%', boxSizing: 'border-box' }}>
        <div style={{ height: 110, marginBottom: 24, borderRadius: 8, background: p.accent === 'var(--cyan)' ? 'rgba(0,212,200,0.05)' : 'rgba(244,98,42,0.05)', border: `1px solid ${p.accent === 'var(--cyan)' ? 'rgba(0,212,200,0.1)' : 'rgba(244,98,42,0.1)'}`, position: 'relative', overflow: 'hidden' }}>
          <span style={{ position: 'absolute', fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 96, color: p.accent === 'var(--cyan)' ? 'rgba(0,212,200,0.07)' : 'rgba(244,98,42,0.07)', lineHeight: 1, bottom: -16, right: 8 }}>{p.num}</span>
          <div style={{ position: 'absolute', inset: 16, display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ flex: 1 }} />
            <div style={{ height: 24, background: p.accent === 'var(--cyan)' ? 'rgba(0,212,200,0.12)' : 'rgba(244,98,42,0.12)', borderRadius: 4, width: '40%' }} />
          </div>
        </div>
